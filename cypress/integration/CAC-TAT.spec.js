...
describe('Central de Atendimento ao Cliente TAT', function() {
    ...
    it('preenche os campos obrigatórios e envia o formulário',function() {
        ...
        cy.clock()
        cy.get('#firstName').should('be.visible').type('Igor')
        cy.get('#lastName').should('be.visible').type('Sousa')
        cy.get('#email').should('be.visible').type('igor.oa.sousa@gmail.com')
    ...
    it('preenche e limpa os campos nome, sobrenome, email e telefone',function() {
        cy.get('#firstName').should('be.visible').type('Igor').should('have.value', 'Igor').clear().should('have.value', '')
        cy.get('#lastName').should('be.visible').type('Sousa').should('have.value', 'Sousa').clear().should('have.value', '')
        cy.get('#email').should('be.visible').type('igor.oa.sousa@gmail.com').should('have.value', 'igor.oa.sousa@gmail.com').clear().should('have.value', '')
        cy.get('#phone').should('be.visible').type('11945132157').should('have.value', '11945132157').clear().should('have.value', '')
        cy.contains('button', 'Enviar').click()
    })
    ...
    it('envia o formuário com sucesso usando um comando customizado',function() {
        ...
        cy.fillMandatoryFieldsAndSubmit('igor', 'sousa', 'igor.oa.sousa@gmail.com', 'teste')
        cy.get('.success').should('be.visible')
        cy.tick(tresSegundos)
        cy.get('.success').should('not.be.visible')
    })
    ...
    it('preenche a area de texto usando o comando invoke', function() {
      //.repeat serve para criar textos longo de forma ctrl+v
      const textoLongo = Cypress._.repeat ('0123456789', 20)
      cy.get('#open-text-area').invoke('val', textoLongo).should('have.value', textoLongo)
    })
